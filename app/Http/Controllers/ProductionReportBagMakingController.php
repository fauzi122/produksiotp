...
<?php
			...
			*/
			...
			$cek_attempt_barcode_start = DB::table('barcode_detail')
				...
				->where(function ($query) {
					$query->where('barcode_number', $_POST['id_master_barcode_start'])
						->orWhere('barcode_number', 'LIKE', $_POST['id_master_barcode_start'] . '%');
				})
				->first(); // gunakan first() untuk mendapatkan 1 record

			// Ambil barcode_number yang sebenarnya dari database (dengan suffix jika ada)
			...
			if (!empty($cek_attempt_barcode_start) && $cek_attempt_barcode_start->used_attempt + $amount_result > 50000) {
				...
				if (!empty($data_slitting[0]->id_report_sfs) && $amount_result > 0  && $pcs_wrap > 0) {
					...
					*/
					...
					if ($hasil_akhir > 0) {
						...
						$validatedData['barcode_start'] = $real_barcode_start; // Gunakan barcode yang sebenarnya dari database
						...
						if (!empty($response)) {
							...
							if (!isset($_POST['used_next_shift']) || isset($_POST['used_next_shift']) || isset($_POST['join'])) {
								$data_barcode_start_update = DB::table('barcode_detail')
									->select('*')
								...
								$updatedDataBS['used_product'] = !empty($data_barcode_start_update->used_product) ? $data_barcode_start_update->used_product : 'FG';
								$updatedDataBS['used_attempt'] = $data_barcode_start_update->used_attempt + $amount_result;
							...
							if ($updatedDataBS['used_product'] == 'FG' and $updatedDataBS['used_attempt'] == '50000') {
								...
								if ($responseUpdateMaster) {
									$data_usage_to = DB::table('report_bag_production_results as a')
	...
	public function production_entry_report_bag_making_detail_production_result_edit_save(Request $request)
	{
		...
		*/
		...
		//echo $cek_attempt_barcode_start[0]->used_attempt;exit;
		...
		if (!empty($data[0])  && isset($_POST['id_master_barcode_start'])) {
			...
			if (!empty($cek_attempt_barcode_start) && $cek_attempt_barcode_start->used_attempt + $amount_result > 50000) {
				return Redirect::to('/production-entry-report-bag-making-detail-production-result-edit/' . $response_id_rb . '/' . $response_id_rb_pr)->with('pesan_danger', 'Barcode Start Tidak Mencukupi');
			} else {
				$pcs_wrap = $_POST['pcs_wrap'];
				...
				$validatedData['start_time'] = $_POST['start'];
				$validatedData['finish_time'] = $_POST['finish'];
				// gunakan barcode yang sebenarnya (dengan suffix bila ada)
				$validatedData['barcode_start'] = !empty($cek_attempt_barcode_start) ? $cek_attempt_barcode_start->barcode_number : $_POST['id_master_barcode_start'];
				$validatedData['barcode'] = $_POST['id_master_barcode'];
				...
				*/
				...
				if ($response) {
					...
					if (!isset($_POST['used_next_shift']) || isset($_POST['used_next_shift']) || isset($_POST['join'])) {
						...
						$updatedDataBS['used_attempt'] = $data_barcode_start[0]->used_attempt + $amount_result;
					...
					if ($validatedData['barcode_start'] <> $data[0]->barcode_start) {	   	/*					
						...
						*/
						...
						$updatedDataBSO['join'] = '-';
						$updatedDataBSO['used_product'] = ($data_barcode_start_old[0]->used_attempt - $data[0]->amount_result) > 0 ? $data_barcode_start_old[0]->used_product : null;
						$updatedDataBSO['used_attempt'] = $data_barcode_start_old[0]->used_attempt - $data[0]->amount_result;

						$update_bso = DB::table('barcode_detail')
							->where('barcode_number', $data[0]->barcode_start)
							->update($updatedDataBSO);

						if ($update_bso and ($updatedDataBSO['used_product'] == 'FG' and $data_barcode_start_old[0]->used_attempt == '50000')) {

							if ($updatedDataBSO['used_product'] == 'FG' and $data_barcode_start_old[0]->used_attempt == '50000') {
						...
						if ($updatedDataBS['used_product'] == 'FG' and $updatedDataBS['used_attempt'] == '50000') {
							...
							if ($responseUpdateMaster) {
								$data_usage_to = DB::table('report_bag_production_results as a')
									->leftJoin('report_bags as b', 'a.id_report_bags', '=', 'b.id')
									->whereRaw("a.barcode_start = '$barcode_start'")
									//->whereRaw( "a.type_result = 'Folding'" )
									->select(
										DB::raw('GROUP_CONCAT(DISTINCT report_number SEPARATOR ", ") AS report_number')
									)
					...
					if ($data[0]->wrap <> $hasil_akhir || $data[0]->pcs_wrap <> $_POST['pcs_wrap']) {
						//HAPUS DETAIL RESULT SEBELUMNYA
	...
	public function production_entry_report_bag_making_detail_production_result_delete(Request $request)
	{
		...
		if (!empty($data[0])) {
			...
			*/
			...
			//Penyesuaian Barcode Start
			//Tambahan
			// Cek barcode dengan suffix (misal: 241000015 atau 241000015FS)
			$data_barcode_start = DB::table('barcode_detail')
				->select('*')
				->where(function ($query) use ($barcode_start) {
					$query->where('barcode_number', $barcode_start)
						->orWhere('barcode_number', 'LIKE', $barcode_start . '%');
				})
				->first(); // gunakan first() bukan get() karena kita butuh 1 data

			//Tambahan
			$data_folding = DB::table('report_sf_production_results as a')
				->leftJoin('report_sfs as b', 'a.id_report_sfs', '=', 'b.id')
				->where(function ($query) use ($barcode_start) {
					$query->where('a.barcode', $barcode_start)
						->orWhere('a.barcode', 'LIKE', $barcode_start . '%');
				})
				->select('a.*', 'b.report_number', 'b.order_name')
				->first(); // gunakan first() bukan get()

			// Cek apakah data_folding ada
			if (!empty($data_folding)) {
				$order_name = explode('|', $data_folding->note);
			} else {
				$order_name = ['', '', ''];
			}

			//Update Barcode Start - Cek apakah data_barcode_start dan data ada
			if (!empty($data_barcode_start) && !empty($data[0])) {

				// Ambil barcode_number yang sebenarnya dari database (bisa dengan suffix)
				$real_barcode_start = $data_barcode_start->barcode_number;

				// Cek apakah masih ada data lain yang menggunakan barcode_start yang sama
				// Gunakan real_barcode_start (dengan suffix jika ada) dan juga cocokkan
				// terhadap variasi tanpa suffix (LIKE) agar record yang tersimpan
				// dalam format berbeda tetap terdeteksi.
				$check_other_usage = DB::table('report_bag_production_results')
					->where(function ($q) use ($barcode_start, $real_barcode_start) {
						$q->where('barcode_start', $real_barcode_start)
							->orWhere('barcode_start', $barcode_start)
							->orWhere('barcode_start', 'LIKE', $barcode_start . '%');
					})
					->whereRaw("sha1(id) != '$id'") // exclude data yang akan dihapus
					->count();

				// Jika tidak ada data lain yang pakai barcode start ini, kembalikan statusnya
				if ($check_other_usage == 0) {
					$updatedDataBS['used_next_shift'] = '1';
					$updatedDataBS['join'] = '-';
					$updatedDataBS['used_product'] = ($data_barcode_start->used_attempt - $data[0]->amount_result) > 0 ? $data_barcode_start->used_product : null;
					$updatedDataBS['used_attempt'] =  $data_barcode_start->used_attempt - $data[0]->amount_result;

					$update_bs = DB::table('barcode_detail')
						->where('barcode_number', $real_barcode_start)
						->update($updatedDataBS);
				} else {
					// Jika masih ada data lain, hanya kurangi used_attempt (jangan ubah used_next_shift)
					$updatedDataBS['used_product'] = ($data_barcode_start->used_attempt - $data[0]->amount_result) > 0 ? $data_barcode_start->used_product : null;
					$updatedDataBS['used_attempt'] =  $data_barcode_start->used_attempt - $data[0]->amount_result;
				...
				if ($update_bs and ($updatedDataBS['used_product'] == 'FG' and $data_barcode_start->used_attempt == '50000')) {

					if ($updatedDataBS['used_product'] == 'FG' and $data_barcode_start->used_attempt == '50000') {
						...
						// Cek apakah data_produk dan data_folding ada
						if (!empty($data_produk) && !empty($data_folding)) {
							$updatedDataMS_BS['stock'] = $data_produk->stock + 1;
							$updatedDataMS_BS['weight_stock'] = $data_produk->weight_stock + $data_folding->weight;
					...
					if (isset($responseUpdateMasterBS) && $responseUpdateMasterBS) {
						DB::table('history_stocks')
							->whereRaw("barcode = '" . $real_barcode_start . "' and type_stock='OUT'")
							->delete();
					}
				}
			}
					...
					if (!empty($data_product[0])) {
						...
						*/
						...
						if ($responseMaster) {
							...
							$responseUpdate = ProductionEntryReportBagMaking::where('report_number', $data_update[0]->report_number)
								->update($validatedData);
				...
				if ($deleteBagMaking) {
					...
					if (!empty($data_detail[0]->barcode)) {
						...
						foreach ($data_detail as $data) {
							...
							$updatedDataBS['used_next_shift'] = '1';
							$updatedDataBS['join'] = '-';
							$updatedDataBS['used_product'] = ($data_barcode_start[0]->used_attempt - $data->amount_result) > 0 ? $data_barcode_start[0]->used_product : null;
							$updatedDataBS['used_attempt'] =  $data_barcode_start[0]->used_attempt - $data->amount_result;

							$update_bs = DB::table('barcode_detail')
								->where('barcode_number', $barcode_start)
								->update($updatedDataBS);

							if ($update_bs and ($updatedDataBS['used_product'] == 'FG' and $data_barcode_start[0]->used_attempt == '50000')) {

								if ($updatedDataBS['used_product'] == 'FG' and $data_barcode_start[0]->used_attempt == '50000') {
