...
<?php
					...
					if (!empty($response)) {
						...
						*/
						...
						if (!isset($_POST['used_next_shift']) || isset($_POST['used_next_shift']) || isset($_POST['join'])) {
							...
							$updatedDataBS['used_product'] = !empty($data_barcode_start[0]->used_product) ? $data_barcode_start[0]->used_product : $type_wo[0];
						...
						if (($updatedDataBS['used_product'] == 'WIP' and $updatedDataBS['used_attempt'] == '8') || ($updatedDataBS['used_product'] == 'FG' and $updatedDataBS['used_attempt'] == '100')) {
							//echo "Disini";
							if ($updatedDataBS['used_product'] == 'WIP' and $updatedDataBS['used_attempt'] == '8') {
								//echo "WIP";
								//$type_wo[1] //id_produk
							...
							if ($updatedDataBS['used_product'] == 'FG' and $updatedDataBS['used_attempt'] == '100') {
								//echo "FG";
								$data_produk = DB::table('master_wips')
									->select('*')
									->where('id', $order_name[1])
							...
							if ($responseUpdateMaster) {
								...
								$validatedData = ([
									'id_good_receipt_notes_details' => $data_blow[0]->report_number,
									...
									'qty' => '1',
									'type_stock' => 'OUT',
									'weight' => $data_blow[0]->weight, //
									'date' => date("Y-m-d"),
									'barcode' => $response->barcode_start,
									'remarks' => 'From Blow Usage To Slitting'
								]);
	...
	public function production_entry_report_slitting_detail_production_result_edit_save(Request $request)
	{
		...
		*/
		...
		if (($type_wo[0] == 'WIP' && $cek_attempt_barcode_start[0]->used_attempt + 1 > 8) || ($type_wo[0] == 'FG' && $cek_attempt_barcode_start[0]->used_attempt + 1 > 100)) {
			return Redirect::to('/production-ent-report-slitting-detail/' . $response_id_rs)->with('pesan', 'Barcode Start Tidak Mencukupi');
		} else {
			//print_r($data);exit;
			if (!empty($data[0])) {
				$pesan = [
				...
				*/
				if ($response) {
					...
					if ($validatedData['barcode_start'] <> $data[0]->barcode_start) {
						...
						$data_blow_barcode_start_old = DB::table('report_blow_production_results as a')
							->leftJoin('report_blows as b', 'a.id_report_blows', '=', 'b.id')
							->whereRaw("a.barcode = '" . $data[0]->barcode_start . "'")
							->select('a.*', 'b.report_number', 'b.order_name')
							->get();

						$order_name_barcode_start_old = explode('|', $data_blow_barcode_start_old[0]->order_name);
						...
						if ($update_bso and (($updatedDataBSO['used_product'] == 'WIP' and $data_barcode_start_old[0]->used_attempt == '8') || ($updatedDataBSO['used_product'] == 'FG' and $data_barcode_start_old[0]->used_attempt == '100'))) {
							if ($updatedDataBSO['used_product'] == 'WIP' and $data_barcode_start_old[0]->used_attempt == '8') {
							...
							if ($updatedDataBSO['used_product'] == 'FG' and $data_barcode_start_old[0]->used_attempt == '100') {
	...
	public function production_entry_report_slitting_detail_production_result_delete(Request $request)
	{
		...
		if (!empty($data[0])) {
			...
			if ($delete) {
				//Start Penyesuaian Barcode END
				//Jika Barcode Bisa Digunakan Lagi, Sesuaikan status data barcode menjadi NULL
				$updatedData['status'] = null;
				DB::table('barcode_detail')
					->where('barcode_number', $barcode)
					->update($updatedData);
				//End Penyesuaian Barcode END
				...
				*/
				...
				$update_bs = DB::table('barcode_detail')
					->where('barcode_number', $barcode_start)
					->update($updatedDataBS);
				//End Penyesuaian Barcode START

				if ($update_bs and (($updatedDataBS['used_product'] == 'WIP' and $data_barcode_start[0]->used_attempt == '8') || ($updatedDataBS['used_product'] == 'FG' and $data_barcode_start[0]->used_attempt == '100'))) {
					if ($updatedDataBS['used_product'] == 'WIP' and $data_barcode_start[0]->used_attempt == '8') {
					...
					if ($updatedDataBS['used_product'] == 'FG' and $data_barcode_start[0]->used_attempt == '100') {
	...
	public function production_entry_report_slitting_update_stock($response_id)
	{
		...
		*/
		...
		$data_update = DB::table('report_sf_production_results AS a')
			...
			->whereRaw("sha1(a.id_report_sfs) = '$id_rs'")
	...
	public function production_entry_report_slitting_unposted($response_id)
	{
		...
		$data_check = ProductionEntryReportSFProductionResult::select('report_sf_production_results.*')
			->rightJoin('report_sf_production_results AS b', 'report_sf_production_results.barcode', '=', 'b.barcode_start')
			->whereRaw("sha1(report_sf_production_results.id_report_sfs) = '$id_rs'")
			->get();

		if ($data_check->isNotEmpty()) {
			return Redirect::to('/production-ent-report-slitting')->with('pesan_danger', 'Report Tidak Dapat Di Unposted, Terdapat Report Folding Yang Menggunakan Barcode Production Result Report Ini.');
		} else {
					...
					if (!empty($data_product[0])) {
						...
						*/
						...
						if ($responseMaster) {
							...
							return Redirect::to('/production-ent-report-slitting')->with('pesan_danger', 'There Is An Error. Kendala Perubahan Data MASTER');
						}
					} else {
						return Redirect::to('/production-ent-report-slitting')->with('pesan_danger', 'There Is An Error. Data Produk Not Found.');
					}
	...
	// AJAX Method untuk Select2 Work Orders
	public function jsonGetWorkOrdersSlitting()
	{
		...
		$query = DB::table('work_orders AS a')
			->leftJoin('sales_orders AS b', 'a.id_sales_orders', '=', 'b.id')
			->leftJoin('master_customers AS c', 'b.id_master_customers', '=', 'c.id')
			->select('a.id', 'a.wo_number', 'a.type_product', 'a.id_master_products', 'c.id AS id_master_customers')
			->whereRaw("left(wo_number,5) = 'WOSLT'");

		if (!empty($search)) {
			$query->where('a.wo_number', 'LIKE', "%{$search}%");
		}
	...
	// AJAX method untuk Select2 Barcode Start Slitting dengan pagination
	public function jsonGetBarcodeStartSlitting(Request $request)
	{
		...
		// Optimized query: use LEFT JOIN instead of NOT EXISTS for better performance
		$query = DB::table('barcode_detail as a')
			->select('a.barcode_number', 'a.join')
			->join('report_blow_production_results as d', 'a.barcode_number', '=', 'd.barcode')
			->join('report_blows as e', 'd.id_report_blows', '=', 'e.id')
			->leftJoin('report_sf_production_results as c', function ($join) {
				$join->on('c.barcode_start', '=', 'a.barcode_number')
					->where('c.type_result', 'Slitting');
			})
			->where('e.status', 'Closed')
			->where('a.status', 'In Stock BLW')
			->where('a.used_next_shift', '1')
			->whereNull('c.id') // Exclude already used in Slitting
			->where(function ($qb) use ($type_product, $attempt) {
				$qb->whereNull('a.used_product');

				if (!empty($type_product)) {
					$qb->orWhere(function ($q2) use ($type_product, $attempt) {
						$q2->where('a.used_product', $type_product);
		...
		if (!empty($search)) {
			$query->where('a.barcode_number', 'LIKE', "%$search%");
		}

		// Group by to handle potential duplicates from joins
		$query->groupBy('a.barcode_number', 'a.join')
			->orderBy('a.barcode_number', 'asc');

		// Get results first with limit+1, then check if more exist (faster)
	...
	// AJAX method untuk Select2 Barcode End Slitting dengan pagination
	public function jsonGetBarcodeEndSlitting(Request $request)
	{
		$search = $request->input('search', '');
		$page = $request->input('page', 1);
		$limit = 10;
