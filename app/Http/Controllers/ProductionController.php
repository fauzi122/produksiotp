	...
	public function production_entry_report_blow_json()
	{
		...
		return DataTables::of($datas)
			...
			->addColumn('order_info', function ($data) {
				...
				$order_info = '<p><b>' . $order_name . '</b><br><code>Customer : ' . $data->name . '</code><br><footer class="blockquote-footer">Status : <cite>' . $status . '</cite>' . $operator . '</footer></p>';
				return $order_info;
			})
	...
	public function production_entry_report_blow_json_update_stock(Request $request)
	{
		...
		*/
		...
		$data = ProductionEntryReportBlowProductionResult::select('b.report_number', 'report_blow_production_results.id_report_blows', 'report_blow_production_results.id')
			...
			//->selectRaw('GROUP_CONCAT(barcode SEPARATOR ', ') AS barcode')
...
<?php
	...
	public function jsonGetProdukAutofill()
	{
		...
		if ($type_product == 'RM') {
			$datas = DB::table('master_raw_materials AS a')
	...
	{
		$where       = request('where');
		$key         = trim((string) request('barcode_number'));
		$typeProduct = request('type_product');
		$barcodeEnd  = request('barcode_end'); // "B001,B002"
		$page        = (string) request('page'); // dipakai sebagai flag aja
		$limit       = (int) (request('limit') ?? 100); // default 100 biar enteng

		// Parse IN list dengan aman and strip surrounding quotes (blade may send "" wrapped values)
		$barcodeEndArr = [];
		if (!empty($barcodeEnd)) {
			$barcodeEndArr = array_values(array_filter(array_map(function ($v) {
				// trim whitespace and both single/double quotes around values
				return trim($v, " \t\n\r\0\x0B'\"");
			}, explode(',', $barcodeEnd))));
		}

		// Build canonical barcode list (match barcode_detail.barcode_number).
		$barcodeEndCanonical = [];
		if (!empty($barcodeEndArr)) {
			foreach ($barcodeEndArr as $v) {
				// exact matches
				$found = DB::table('barcode_detail')->where('barcode_number', $v)->pluck('barcode_number')->toArray();
				// if none found, try prefix match (handles short keys like '241000016' -> '241000016FS')
				if (empty($found)) {
					$found = DB::table('barcode_detail')->where('barcode_number', 'like', $v . '%')->pluck('barcode_number')->toArray();
				}
				if (!empty($found)) {
					$barcodeEndCanonical = array_merge($barcodeEndCanonical, $found);
				}
			}
			$barcodeEndCanonical = array_values(array_unique($barcodeEndCanonical));
		}

		// Filter canonical list by the latest entry in report_bag_production_results
		// Only exclude if the most recent record for that barcode has used_next_shift_end = '0'
		if (!empty($barcodeEndCanonical)) {
			// pick latest by created_at timestamp per barcode
			$latestExcluded = DB::table('report_bag_production_results as r1')
				->join(DB::raw("(SELECT barcode, MAX(created_at) AS max_created FROM report_bag_production_results GROUP BY barcode) r2"), function ($join) {
					$join->on('r1.barcode', '=', 'r2.barcode')
						->on('r1.created_at', '=', DB::raw('r2.max_created'));
				})
				->where('r1.used_next_shift_end', '0')
				->whereIn('r1.barcode', $barcodeEndCanonical)
				->pluck('r1.barcode')
				->toArray();

			if (!empty($latestExcluded)) {
				$barcodeEndCanonical = array_values(array_unique($latestExcluded));
			} else {
				// nothing to exclude after checking latest detail
				$barcodeEndCanonical = [];
			}
		}

		// Prepare debug variables (may be filled in some branches)
		$validBarcodes = [];
		$usedBarcodes = [];
		// barcode_starts excluded by latest report_bag_production_results (created_at) with used_next_shift = '0'
		$usedStartBarcodes = [];

		// If the caller provided a short key (e.g. "241000015" while DB uses "241000015FS"),
		// try to canonicalize it against barcode_detail so membership checks use the
		// full barcode values stored in the DB.
		$keyCanonical = [];
		if ($key !== '') {
			// exact match first
			$found = DB::table('barcode_detail')->where('barcode_number', $key)->pluck('barcode_number')->toArray();
			if (empty($found)) {
				// fallback to prefix match
				$found = DB::table('barcode_detail')->where('barcode_number', 'like', $key . '%')->pluck('barcode_number')->toArray();
			}
			$keyCanonical = array_values(array_unique($found));
		}

		// Base query: select id dan barcode_number untuk ordering by id DESC
		$q = DB::table('barcode_detail as a')->select('a.id', 'a.barcode_number')->distinct();

		// Helper: filter by optional keyword
		$applyKey = function ($qb) use ($key) {
			if ($key !== '') {
				$qb->where('a.barcode_number', $key);
			}
		};
		...
		switch ($where) {
			case 'BLOW': {
					$q->join('barcodes as b', 'a.id_barcode', '=', 'b.id')
						->whereNull('a.status')
						->where('b.id_master_process_productions', 2)
						->when($key !== '', function ($qb) use ($key) {
							// hindari OR di luar grup, pakai grup agar index tetap kepakai
							$qb->orWhere('a.barcode_number', $key);
						});
					break;
				}

			case 'SLITTING START': {
					$attempt = ($typeProduct === 'WIP') ? 8 : 100;

					// e.status = 'Closed' (via relasi blows)
					$q->join('report_blow_production_results as d', 'a.barcode_number', '=', 'd.barcode')
						->join('report_blows as e', 'd.id_report_blows', '=', 'e.id')
						->where('e.status', 'Closed')
						->where('a.status', 'In Stock BLW')
						->where('a.used_next_shift', '1') // string '1' sesuai datamu
						->where(function ($qb) use ($typeProduct, $key, $page, $attempt) {
							// grup 1: belum pernah diset used_product
							$qb->whereNull('a.used_product');

							// grup 2: filter berdasarkan type_product & attempt
							if (!empty($typeProduct)) {
								$qb->orWhere(function ($q2) use ($typeProduct, $key, $page, $attempt) {
									$q2->where('a.used_product', $typeProduct);
									if ($key !== '') {
										$q2->where('a.barcode_number', $key);
									}
									if ($page !== '') { // flag untuk apply attempt
										$q2->where('a.used_attempt', '<', $attempt);
									}
								});
							}
						})
						// Anti-join: belum dipakai sebagai barcode_start untuk Slitting
						->whereNotExists(function ($sub) {
							$sub->from('report_sf_production_results as c')
								->whereColumn('c.barcode_start', 'a.barcode_number')
								->where('c.type_result', 'Slitting')
								->select(DB::raw(1));
						});

					// Exclude barcode_start values where the latest report_bag_production_results
					// record (by created_at) has used_next_shift = '0'. Use a whereNotExists
					// SQL subquery that matches latest barcode_start rows and excludes any
					// candidate 'a.barcode_number' that equals or starts with that barcode_start.
					// Also keep an expanded canonical array for debug purposes.
					$usedStartBarcodes = DB::table('report_bag_production_results as r1')
						->join(DB::raw("(SELECT barcode_start, MAX(created_at) AS max_created FROM report_bag_production_results GROUP BY barcode_start) r2"), function ($join) {
							$join->on('r1.barcode_start', '=', 'r2.barcode_start')
								->on('r1.created_at', '=', DB::raw('r2.max_created'));
						})
						->where('r1.used_next_shift', '0')
						->pluck('r1.barcode_start')
						->toArray();

					// Expand to canonical forms for debug and for edit-time membership checks
					$usedStartBarcodesExpanded = [];
					if (!empty($usedStartBarcodes)) {
						foreach ($usedStartBarcodes as $s) {
							$usedStartBarcodesExpanded[] = $s;
							$more = DB::table('barcode_detail')
								->where('barcode_number', 'like', $s . '%')
								->pluck('barcode_number')
								->toArray();
							if (!empty($more)) {
								$usedStartBarcodesExpanded = array_merge($usedStartBarcodesExpanded, $more);
							}
						}
						$usedStartBarcodesExpanded = array_values(array_unique($usedStartBarcodesExpanded));
					}

					// Exclude a.barcode_number when the latest report_bag_production_results
					// row (among rows whose barcode_start is a prefix of a.barcode_number)
					// has used_next_shift = '0'. If the caller provided a full canonical
					// barcode key, prefer to match barcode_start by exact equality (parameterized)
					// instead of doing a prefix LIKE. Otherwise fall back to the previous
					// prefix-based latest-row logic.
					if ($key !== '' && !empty($keyCanonical)) {
						// Use the first canonical form as the full barcode_start to check.
						$startParam = $keyCanonical[0];
						$q->whereNotExists(function ($sub) use ($startParam) {
							$sub->from('report_bag_production_results as r1')
								->where('r1.used_next_shift', '0')
								->whereRaw("r1.created_at = (
									SELECT MAX(r3.created_at) FROM report_bag_production_results r3
									WHERE r3.barcode_start = ?
								)", [$startParam])
								->select(DB::raw(1));
						});
					} else {
						$q->whereNotExists(function ($sub) {
							$sub->from('report_bag_production_results as r1')
								->where('r1.used_next_shift', '0')
								// r1 must be the latest row among those whose barcode_start is a prefix
								// of the outer candidate a.barcode_number
								->whereRaw("r1.created_at = (
									SELECT MAX(r3.created_at) FROM report_bag_production_results r3
									WHERE a.barcode_number LIKE CONCAT(r3.barcode_start, '%')
								)")
								->select(DB::raw(1));
						});
					}
			...
			case 'SLITTING': {
					$q->join('barcodes as b', 'a.id_barcode', '=', 'b.id')
						->whereNull('a.status')
						->where('b.id_master_process_productions', 4)
						->when($key !== '', fn($qb) => $qb->orWhere('a.barcode_number', $key));
					break;
				}
			...
			case 'FOLDING START': {
					...
					// e.status Closed dari proses SLITTING START
					$q->join('report_sf_production_results as d', function ($join) {
						...
						->where(function ($qb) use ($key, $attempt, $page) {
							$qb->whereNull('a.used_product')
								->orWhere(function ($q2) use ($key, $attempt, $page) {
									if ($key !== '') {
										$q2->where('a.barcode_number', $key);
									}
									if ($page !== '') {
								...
								->orWhere('a.used_attempt', '<', $attempt);
						})
						// Belum dipakai sebagai barcode_start untuk Folding
						->whereNotExists(function ($sub) {
							$sub->from('report_sf_production_results as c')
								->whereColumn('c.barcode_start', 'a.barcode_number')
			...
			case 'FOLDING': {
					$q->join('barcodes as b', 'a.id_barcode', '=', 'b.id')
						->whereNull('a.status')
						->where('b.id_master_process_productions', 3)
						->when($key !== '', fn($qb) => $qb->orWhere('a.barcode_number', $key));
					break;
				}
			...
			case 'BAG START': {
					...
					// Step 1: Get valid barcodes from Slitting (Closed status)
					$validBarcodes = DB::table('report_sf_production_results as d')
						->join('report_sfs as e', 'd.id_report_sfs', '=', 'e.id')
						->where('d.type_result', 'Slitting')
						->where('e.status', 'Closed')
						->pluck('d.barcode')
					...
					// If a short barcode key was provided, also include any slitting barcodes that start with it
					if ($key !== '') {
						$more = DB::table('report_sf_production_results as d')
							->join('report_sfs as e', 'd.id_report_sfs', '=', 'e.id')
							->where('d.type_result', 'Slitting')
							->where('e.status', 'Closed')
							->where('d.barcode', 'like', $key . '%')
							->pluck('d.barcode')
							->toArray();

						if (!empty($more)) {
							$validBarcodes = array_values(array_unique(array_merge($validBarcodes, $more)));
						}
					}

					// Step 2: Get already used barcodes in Bag Making
					// Check report_bag_production_results with MAX(id) and used_next_shift = '0'
					$usedBarcodes = DB::table('report_bag_production_results as rbpr1')
						->whereRaw('rbpr1.id = (
							SELECT MAX(rbpr2.id)
							FROM report_bag_production_results rbpr2
							WHERE rbpr2.barcode_start = rbpr1.barcode_start
						)')
						->where('rbpr1.used_next_shift', '0')
						->pluck('rbpr1.barcode_start')
						->toArray();

					// Step 3: Main query with whereIn/whereNotIn (much faster)
					$q->whereIn('a.status', ['In Stock SLT FG', 'In Stock FLD'])
						->where('a.used_next_shift', '1')
						->whereIn('a.barcode_number', $validBarcodes)
						->whereNotIn('a.barcode_number', $usedBarcodes)
						->where(function ($qb) use ($key, $attempt, $page) {
							$qb->whereNull('a.used_product')
								->orWhere(function ($q2) use ($key, $attempt, $page) {
									if ($key !== '') {
					...
					// Jika ada barcode_number spesifik (misal saat edit), pastikan barcode tersebut tetap dimasukkan
					// HANYA jika barcode tersebut TIDAK ada dalam daftar exclusion (used_next_shift = '0')
					if ($key !== '') {
						// Check if the requested barcode (or its canonical forms) is in the exclusion list
						$isExcludedByStart = false;

						if (!empty($usedBarcodes)) {
							// Check direct key
							if (in_array($key, $usedBarcodes, true)) {
								$isExcludedByStart = true;
							}

							// Check canonical forms of the requested key
							if (!$isExcludedByStart && !empty($keyCanonical)) {
								foreach ($keyCanonical as $kc) {
									if (in_array($kc, $usedBarcodes, true)) {
										$isExcludedByStart = true;
										break;
									}
								}
						...
						// ONLY re-include if NOT excluded (used_next_shift != '0')
						if (!$isExcludedByStart) {
							// Accept exact match or prefix match so short form (without suffix) still shows the full barcode
							$q->orWhere(function ($qb) use ($key) {
								$qb->where('a.barcode_number', $key)
									->orWhere('a.barcode_number', 'like', $key . '%');
							});
						}
					}
					break;
			...
			case 'BAG': {
					...
					if (!empty($excludeList)) {
						$q->whereNotIn('a.barcode_number', $excludeList);
					}

					// Exclude barcode where latest report_bag_production_results has used_next_shift_end = '0'
					// Check by MAX(id) not created_at
					$usedEndBarcodes = DB::table('report_bag_production_results as rbpr1')
						->whereRaw('rbpr1.id = (
							SELECT MAX(rbpr2.id)
							FROM report_bag_production_results rbpr2
							WHERE rbpr2.barcode = rbpr1.barcode
						)')
						->where('rbpr1.used_next_shift_end', '0')
						->pluck('rbpr1.barcode')
						->toArray();

					if (!empty($usedEndBarcodes)) {
						$q->whereNotIn('a.barcode_number', $usedEndBarcodes);
					}

					// Jika ada barcode_number spesifik (untuk edit), tetap tampilkan
					// HANYA jika barcode tersebut TIDAK ada dalam daftar exclusion (used_next_shift_end = '0')
					if ($key !== '') {
						// Check if the requested barcode (or its canonical forms) is in the exclusion list
						$isExcludedByEnd = false;

						// Check against usedEndBarcodes
						if (!empty($usedEndBarcodes)) {
							// Check direct key
							if (in_array($key, $usedEndBarcodes, true)) {
								$isExcludedByEnd = true;
							}

							// Check canonical forms of the requested key
							if (!$isExcludedByEnd && !empty($keyCanonical)) {
								foreach ($keyCanonical as $kc) {
									if (in_array($kc, $usedEndBarcodes, true)) {
										$isExcludedByEnd = true;
										break;
									}
								}
						...
						// Also check against excludeList (barcodes already in current session)
						if (!$isExcludedByEnd && !empty($excludeList)) {
							...
							if (!$isExcludedByEnd && !empty($keyCanonical)) {
								foreach ($keyCanonical as $kc) {
									if (in_array($kc, $excludeList, true)) {
										$isExcludedByEnd = true;
										break;
									}
								}
						...
						// ONLY re-include if NOT excluded (used_next_shift_end != '0')
						if (!$isExcludedByEnd) {
							$q->orWhere(function ($qb) use ($key, $keyCanonical) {
								// include exact raw key
								$qb->where('a.barcode_number', $key);

								// include canonical full barcode(s) if available
								if (!empty($keyCanonical)) {
									$qb->orWhereIn('a.barcode_number', $keyCanonical);
								} else {
									// fallback: include any barcode that starts with the short key
									$qb->orWhere('a.barcode_number', 'like', $key . '%');
								}
							});
						}
					}
			...
			default: {
					// fallback; tetap batasi
					// NOTE: bila mau cepat, hindari select * dan join
					// & pastikan ada index pada barcode_number
				}
		}

		// Batasi hasil dan urutkan berdasarkan id DESC (ambil id terbesar)
		$results = $q->orderBy('a.id', 'desc')
			->limit($limit)
			->get();

		// Render HTML option (escape biar anti-XSS)
		...
		foreach ($results as $result) {
			...
			$safe = htmlspecialchars($barcode, ENT_QUOTES, 'UTF-8');
			$lists .= "<option value='{$safe}' {$selected}>{$safe}</option>";
		}

		// Extract barcode numbers for debug output
		$barcodes = collect($results)->pluck('barcode_number');

		// If debug flag present, return intermediate arrays and counts
		if (request()->get('debug') == '1') {
			// Build detailed latest-row debug for each canonical barcode_end
			$barcodeEndLatest = [];
			if (!empty($barcodeEndCanonical)) {
				$rows = DB::table('report_bag_production_results as r1')
					->join(DB::raw("(SELECT barcode, MAX(created_at) AS max_created FROM report_bag_production_results GROUP BY barcode) r2"), function ($join) {
						$join->on('r1.barcode', '=', 'r2.barcode')
							->on('r1.created_at', '=', DB::raw('r2.max_created'));
					})
					->whereIn('r1.barcode', $barcodeEndCanonical)
					->select('r1.barcode', 'r1.id', 'r1.created_at', 'r1.used_next_shift_end', 'r1.used_next_shift')
					->get();

				foreach ($rows as $r) {
					$barcodeEndLatest[$r->barcode] = [
						'id' => $r->id,
						'created_at' => $r->created_at,
						'used_next_shift_end' => $r->used_next_shift_end,
						'used_next_shift' => $r->used_next_shift,
					];
				}
			}

			$debug = [
				'barcode_end_arr' => array_values($barcodeEndArr),
				'barcode_end_canonical' => array_values($barcodeEndCanonical),
				'barcode_end_latest_rows' => $barcodeEndLatest,
				'validBarcodes' => array_slice($validBarcodes, 0, 200),
				'usedBarcodes' => array_slice($usedBarcodes, 0, 200),
				'usedStartBarcodes' => array_values($usedStartBarcodes),
				'usedStartBarcodesExpanded' => isset($usedStartBarcodesExpanded) ? array_values($usedStartBarcodesExpanded) : [],
				'barcodes_returned' => array_values($barcodes->toArray()),
				'counts' => [
					'barcodeEndArr' => count($barcodeEndArr),
					'barcodeEndCanonical' => count($barcodeEndCanonical),
					'validBarcodes' => count($validBarcodes),
					'usedBarcodes' => count($usedBarcodes),
					'returned' => count($barcodes),
				],
			];

			return response()->json(['list_barcode' => $lists, 'debug' => $debug]);
		}

		return response()->json(['list_barcode' => $lists]);
	}
	...
	public function production_entry_report_blow_update_stock($response_id)
	{
		...
		$data_update = ProductionEntryReportBlowProductionResult::select('b.report_number','c.type_product','b.order_name','report_blow_production_results.id_report_blows', 'report_blow_production_results.id')
			...
			->selectRaw('SUM(IF(report_blow_production_results.status="Hold", 1, 0)) AS hold')
			->selectRaw('SUM(IF(report_blow_production_results.status="Reject", 1, 0)) AS reject')
			->selectRaw('GROUP_CONCAT((report_blow_production_results.barcode="Good") SEPARATOR ', ') AS barcode_good')
			->rightJoin('report_blows AS b', 'report_blow_production_results.id_report_blows', '=', 'b.id')
			...
			->get();
		*/
		$data_update = DB::table('report_blow_production_results AS a')
			->rightJoin('report_blows AS b', 'a.id_report_blows', '=', 'b.id')
			->rightJoin('work_orders AS c', 'b.id_work_orders', '=', 'c.id')
			->select(
				'b.report_number',
				'c.type_product',
			...
			->whereRaw("sha1(a.id_report_blows) = '$id_rb'")
			->groupBy('a.id_report_blows')
			->get();
	...
	public function production_entry_report_blow_unposted($response_id)
	{
		...
		$data_check = ProductionEntryReportBlowProductionResult::select('report_blow_production_results.*')
			->rightJoin('report_sf_production_results AS b', 'report_blow_production_results.barcode', '=', 'b.barcode_start')
			->whereRaw("sha1(report_blow_production_results.id_report_blows) = '$id_rb'")
			->get();

		if ($data_check->isNotEmpty()) {
			return Redirect::to('/production-ent-report-blow')->with('pesan_danger', 'Report Tidak Dapat Di Unposted, Terdapat Report Slitting Yang Menggunakan Barcode Production Result Report Ini.');
		} else {
			/*QUERY LAMA SEBELUM INSERT BARCODE DENGAN GROUP CONCAT
			$data_update = ProductionEntryReportBlowProductionResult::select('b.report_number','c.type_product','b.order_name','report_blow_production_results.id_report_blows', 'report_blow_production_results.id')
				->selectRaw('SUM(IF(report_blow_production_results.status="Good", 1, 0)) AS good')
