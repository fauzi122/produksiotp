...
<?php
	...
	// AJAX method untuk Select2 Work Orders dengan pagination
	public function jsonGetWorkOrdersFolding(Request $request)
	{
		...
		if (!empty($search)) {
			$query->whereRaw("(a.wo_number LIKE '%$search%' OR c.name LIKE '%$search%')");
		}
		...
		foreach ($results as $row) {
			$data[] = [
				'id' => $row->id,
				'text' => $row->wo_number,
				'id_master_customers' => $row->id_master_customers,
				'type_product' => $row->type_product,
				'id_master_products' => $row->id_master_products
			];
		}
	...
	// AJAX method untuk Select2 Barcode Start Folding dengan pagination
	public function jsonGetBarcodeStartFolding(Request $request)
	{
		$search = $request->input('search', '');
		$page = $request->input('page', 1);
		$limit = 5;
		...
		// Ultra-optimized query: Start from barcode_detail (smaller, filtered table)
		// Use simple WHERE conditions instead of complex JOINs
		$query = DB::table('barcode_detail as a')
			->select('a.barcode_number')
			->where('a.status', 'In Stock SLT WIP')
			->where('a.used_next_shift', '1')
			->where(function ($qb) use ($attempt) {
				$qb->whereNull('a.used_product')
					->orWhere('a.used_attempt', '<', $attempt);
			})
			// Check if barcode exists in valid Slitting results (Closed status)
			->whereExists(function ($sub) {
				$sub->from('report_sf_production_results as d')
					->join('report_sfs as e', 'd.id_report_sfs', '=', 'e.id')
					->whereColumn('d.barcode', 'a.barcode_number')
					->where('d.type_result', 'Slitting')
					->where('e.status', 'Closed');
			})
			// Check if NOT already used in Folding
		...
		$query->orderBy('a.barcode_number', 'asc');

		// Get results with limit+1 to check if there's more
	...
	// AJAX method untuk Select2 Barcode End Folding dengan pagination
	public function jsonGetBarcodeEndFolding(Request $request)
	{
		$search = $request->input('search', '');
		$page = $request->input('page', 1);
		$limit = 10;
	...
	public function production_entry_report_folding_detail_json($response_id)
	{
		...
		return DataTables::of($query)
			...
			->addColumn('aksi', function ($row) use ($response_id) {
				$idSha = sha1($row->id);
				$csrf = csrf_field();
				$deleteHtml = '<form action="/production-entry-report-folding-detail-production-result-delete" method="post" class="d-inline" enctype="multipart/form-data">' . $csrf . '<input type="hidden" name="token_rf" value="' . $response_id . '"><button type="submit" class="btn btn-danger" onclick="return confirm(\'Are you sure to delete this item ?\')" value="' . $idSha . '" name="hapus_detail"><i class="bx bx-trash-alt" title="Delete"></i></button></form>';
				$editHtml = '<a href="/production-entry-report-folding-detail-production-result-edit/' . $response_id . '/' . $idSha . '" class="btn btn-info waves-effect waves-light"><i class="bx bx-edit-alt" title="Edit"></i></a>';
