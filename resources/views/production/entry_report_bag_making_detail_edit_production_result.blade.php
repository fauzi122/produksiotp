	...
	<div class="container-fluid">
		...
		<script>
			$(document).ready(function(){
                                    			<?php
                                    				$noteParts = explode('|', $data[0]->note ?? '');
                                    				$typeProduct = isset($noteParts[0]) ? $noteParts[0] : '';
                                    				$idMasterProd = isset($noteParts[1]) ? $noteParts[1] : '';
                                    			?>
                                    			$.ajax({
                                    				type: "GET",
                                    				url: "/json_get_produk",
                                    				data: { type_product : {!! "'".$typeProduct."'" !!}, id_master_products : {!! $idMasterProd !!} },//baru sampe sinihhh
                                    				dataType: "json",
			...
			<div class="col-lg-6">
				<div class="card">
					...
					<div class="card-body p-4">
						<form method="post"
							action="/production-entry-report-bag-making-detail-production-result-edit-save"
							...
							<div class="alert alert-info alert-dismissible alert-label-icon label-arrow fade show"
								...
								kamu melakukan perubahan bungkus di bagian <b>WRAP</b>, kamu harus
								melakukan penyesuaian kembali di data <b>WRAP Detail</b>.
								<button type="button" class="btn-close" data-bs-dismiss="alert"
									aria-label="Close"></button>
							</div><br>
							<div class="row mb-4 field-wrapper required-field">
								<label for="horizontal-password-input" class="col-sm-4 col-form-label">Work Orders
								</label>
								<div class="col-sm-8">
									<select class="form-select data-select2" name="id_work_orders" id="id_work_orders"
										...
										<option value="{{ $data_for->id }}"
											data-id_master_customers="{{ $data_for->id_master_customers }}"
											data-type_product="{{ $data_for->type_product }}"
											data-id_master_products="{{ $data_for->id_master_products }}"
											data-wo_number="{{ $data_for->wo_number }}" {{ $data_for->id
											== $data[0]->id_work_orders ? 'selected' : '' }}>{{
											$data_for->wo_number }}</option>
										@endforeach
									</select>
							...
							<div class="row mb-4 field-wrapper">
								<label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Product Info
								</label>
								<div class="col-sm-8">
									...
									<div class="text-danger"><b>{{ $errors->first('id_master_products')
											}}</b></div>
									@endif
								</div>
							</div>
							<div class="row mb-4 field-wrapper required-field">
								<label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Start Time
								</label>
								<div class="col-sm-8">
							...
							<div class="row mb-4 field-wrapper required-field">
								<label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Finish Time
								</label>
								<div class="col-sm-8">
									<input type="time" class="form-control" name="finish" id="finish"
										value="{{ $data[0]->finish_time; }}">
							...
							<div class="row mb-4 field-wrapper required-field">
								<label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Barcode
									Start</label>
								<div class="col-sm-8">
									<select class="form-select data-select2" name="id_master_barcode_start"
							...
							<script>
								$(document).ready(function(){
													...
													var _data_bag_start_edit = { where : 'BAG START', barcode_number : {!! "'".$data[0]->barcode_start."'" !!}, wo_number : $('#id_work_orders option:selected').attr('data-wo_number') };
													$.ajax({
														type: "GET",
							...
							<div class="row mb-4 field-wrapper">
								...
								<div class="col-sm-8 mt-3">
									<label class="toggleSwitch nolabel" onclick="">
										<input type="checkbox" name="used_next_shift" <?=$data[0]->used_next_shift ==
										'1' ? 'checked' : '' ?>/>
										<a></a>
										<span>
							...
							<div class="row mb-4 field-wrapper">
								...
								<div class="col-sm-8">
									<select class="form-select data-select2" name="join" id="join">
										...
										<option value="1" {{ $data[0]->join=='1'?'selected':'' }}>1
										</option>
										<option value="2" {{ $data[0]->join=='2'?'selected':'' }}>2
										</option>
										<option value="3" {{ $data[0]->join=='3'?'selected':'' }}>3
										</option>
									</select>
									@if($errors->has('join'))
							...
							<div class="row mb-4 field-wrapper">
								...
								<div class="col-sm-8 mt-3">
									<label class="toggleSwitch nolabel" onclick="">
										<input type="checkbox" name="used_next_shift_barcode"
											<?=$data[0]->used_next_shift_barcode == '1' ? 'checked' : ''
										?>/>
										<a></a>
										<span>
											<span class="left-span">No</span>
							...
							<div class="row mb-4 field-wrapper required-field">
								<label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Weight Starting
								</label>
								<div class="col-sm-8">
									<input type="text" class="form-control" name="weight_starting"
										value="{{ $data[0]->weight_starting; }}">
							...
							<div class="row mb-4 field-wrapper required-field">
								<label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Amount Result
								</label>
								<div class="col-sm-8">
									<input type="text" class="form-control" name="amount_result" id="amount_result"
										value="{{ $data[0]->amount_result; }}">
							...
							<div class="row mb-4 field-wrapper required-field">
								<label for="horizontal-firstname-input" class="col-sm-4 col-form-label mt-4">PCS Per
									Wrap </label>
								<div class="col-sm-8">
									<span class="badge bg-secondary-subtle text-secondary">
							...
							<div class="row mb-4 field-wrapper">
								<label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Wrap Note
								</label>
								<div class="col-sm-8">
									<textarea rows="5" class="form-control"
										name="keterangan">{{ $data[0]->keterangan; }}</textarea>
							...
							<div class="row mb-4 field-wrapper">
								<label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Cause Waste
								</label>
								<div class="col-sm-8">
									<input type="text" class="form-control" name="cause_waste"
										value="{{ $data[0]->cause_waste; }}">
							...
							<input type="hidden" class="form-control" name="token_rb_pr"
								value="{{ sha1($data[0]->id); }}">
				...
				<div class="card">
					...
					<div class="card-body p-4">
						...
						<div class="alert alert-danger alert-dismissible alert-label-icon label-arrow fade show"
							role="alert">
							<i
								class="mdi mdi-alert-octagon-outline label-icon"></i><strong>Perhatian</strong><br>Terdapat
							ketidaksesuaian data <b>WRAP Detail ( {{count($data_detail)}} )</b> dengan jumlah <b>WRAP (
								{{$data[0]->wrap}} )</b>. Silahkan perbaharui data.
							<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
						</div>
						@endif
						@if(count($data_detail) < $data[0]->wrap)
							<button data-bs-toggle="modal" data-bs-target="#addItemModal"
								class="mb-3 btn btn-success waves-effect waves-light" name="tambah" id="tambah">
										...
										<form action="/production-entry-report-bag-making-wrap-add" method="POST">
											...
											<div class="modal-body">
												<div class="mb-3 required-field">
													<label for="description" class="form-label">Jumlah Per Bungkus
														(Wrap)</label>
											...
											<div class="modal-footer">
												<input type="hidden" class="form-control" name="token_rb"
							...
							<div class="table-responsive">
								<table id="datatable" class="table table-bordered dt-responsive  nowrap w-100">
									...
									<tbody>
										...
										<tr>
											<td>
												<div class="btn btn-{{ !empty($data_detail->wrap_pcs)?" dark":"warning"
											...
											<td>
												<center>
													<form action="/production-entry-report-bag-making-wrap-delete"
														...
														<input type="hidden" class="form-control"
															name="token_rb_pr_detail"
															value="{{ sha1($data_detail->id); }}">

														<button type="submit" class="btn btn-danger"
															onclick="return confirm('Are you sure to delete this item ?')"
															value="{{ sha1($data_detail->id) }}" name="hapus_detail">
															<i class="bx bx-trash-alt" title="Hapus"></i>
														</button>
													</form>
													<button data-bs-toggle="modal"
														data-bs-target="#editItemModal{{$data_detail->id}}"
														class="btn btn-info waves-effect waves-light" name="edit"
														id="edit{{$data_detail->id}}">
										...
										<!-- Modal Edit-->
										<div class="modal fade" id="editItemModal{{$data_detail->id}}" tabindex="-1"
											aria-labelledby="editItemModalLabel" aria-hidden="true">
											<div class="modal-dialog">
												<div class="modal-content">
													<div class="modal-header">
														<h5 class="modal-title" id="editItemModalLabel">Edit Wrap Detail
														</h5>
														<button type="button" class="btn-close" data-bs-dismiss="modal"
															aria-label="Close"></button>
													</div>
													<form action="/production-entry-report-bag-making-wrap-edit"
														method="POST">
														@csrf
														<div class="modal-body">
															<div class="mb-3 required-field">
																<label for="description" class="form-label">Jumlah Per
																	Bungkus (Wrap)</label>
																<input type="text" class="form-control" name="wrap_pcs"
																	value="{{ $data_detail->wrap_pcs }}">
																<div class="text-secondary"><b>Pcs</b></div>
															</div>
															<div class="mb-3">
